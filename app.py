...
send_email = SendReport2Email().send_email_via_smtp  # Assign the method, not the result of calling it
...
def generate_summarized_report(research_results):
    ...
    try:
        ...
        for plant, results in research_results.items():
            ...
            summarized_report += f"- Health Benefits: {results.get('health', 'No data available')}\n"
            summarized_report += f"- Growing Season: {results.get('season', 'No data available')}\n"
            summarized_report += f"- Market Prices: {results.get('price', 'No data available')}\n\n"
